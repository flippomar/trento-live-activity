...
/* ===================== QUICK FILTERS ===================== */
function QuickFilters({ active, setActive }: any) {
  return (
...
/* ===================== TRENDING TONIGHT ===================== */
function TrendingTonight({ onPick }: any) {
  return (
...
function PostCard({ e, liked, saved, onLike, onSave, onOpen, flash }: any) {
  ...
  const catLabel = tlaCatLabel(e.cat);
...
/* ===================== FEED ===================== */
const Feed = React.forwardRef<any, any>(function Feed({ filter, likes, saves, onLike, onSave, onOpen, flashId }, ref) {
    ...
    <div className="ev-col feed" ref={ref}>
      ...
      {list.map((e) => (
        <div className="feed-row" key={e.id}>
          <div className="tl">
            <span className={"tl-node" + (e.live ? " live" : "")} style={{ "--tc": catColor(e.cat) }}></span>
...
function NextActivity({ joined, saved, onJoin, onSave }: any) {
  const e = EV_NEXT;
          ...
          return (
            <button className="mini" key={e.id} style={{ "--mc": color, "--mimg": EV_GRAD[e.cat] }} onClick={() => onOpen(e.id)}>
              ...
              <div className="mini-body">
                ...
                <div className="mini-cat"><Icon name={CAT_ICON[e.cat]} size={11} />{tlaCatLabel(e.cat)}</div>
              </div>
            </button>
          );
        })}
    ...
    <div className="modal-scrim" onClick={onClose}>
      <div className="modal" style={{ "--mc": color, "--mimg": EV_GRAD[e.cat] }} onClick={(ev) => ev.stopPropagation()}>
        <div className="modal-media">
          ...
          <span className="mm-tag"><span className="pc-ic" style={{ color }}><Icon name={CAT_ICON[e.cat]} size={13} /></span>{tlaCatLabel(e.cat)}</span>
        </div>
