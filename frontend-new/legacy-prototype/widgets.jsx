...
const CAT_ICON = { musica: "music", cultura: "landmark", sport: "run", cibo: "food", outdoor: "bike", famiglia: "family" };
...
function Widget({ title, accent, upd, children, style, delay }) {
...
/* ---------------- MARKERS ---------------- */
function MarkersLayer({ active, onFocus, popup, onClosePopup }) {
  const { MARKERS, catColor, catLabel } = window.TLA;
  return (
...
/* ---------------- WEATHER ---------------- */
function WeatherWidget({ delay }) {
  const { WEATHER } = window.TLA;
  return (
...
/* ---------------- ALERTS ---------------- */
function AlertsWidget({ delay }) {
  const { ALERTS } = window.TLA;
...
function ParkingWidget({ delay }) {
  const { PARKING } = window.TLA;
...
/* ---------------- ACTIVE AREAS ---------------- */
function ActiveAreasWidget({ delay }) {
  const { AREAS } = window.TLA;
  return (
...
/* ---------------- EVENTS ---------------- */
function EventsWidget({ delay, onFocus }) {
  const { EVENTS, catColor } = window.TLA;
  return (
...
Object.assign(window, {
  useGlow, Widget, MapLabels, MarkersLayer,
  WeatherWidget, AlertsWidget, ParkingWidget, ActiveAreasWidget, EventsWidget,
});
